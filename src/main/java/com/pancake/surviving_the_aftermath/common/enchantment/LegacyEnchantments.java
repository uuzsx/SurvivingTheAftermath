...
public final class LegacyEnchantments {
    ...
    public static void afterDamage(LivingDamageEvent.Post event) {
        ...
        if (event.getSource().getEntity() instanceof Player player) {
            player.heal(event.getNewDamage() * .05F * level(player.getMainHandItem(), "bloodthirsty"));
        }
        var target = event.getEntity();
    ...
    public static void attributes(ItemAttributeModifierEvent event) {
        ...
        int rank = level(stack, "devoured");
        float amount = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getFloat(DEVOURED_DATA);
        ...
        if (life > 0 && equippable != null && List.of(EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET).contains(equippable.getEquipmentSlot())) {
