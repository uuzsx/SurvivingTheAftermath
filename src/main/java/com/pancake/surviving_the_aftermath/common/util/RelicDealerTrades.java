...
public final class RelicDealerTrades {
    ...
    public static boolean isDealer(Villager villager) {
        return villager.getVillagerData().profession().value() == ModVillagers.RELIC_DEALER.get();
    }

    public static void synchronize(Villager villager) {
        if (!isDealer(villager)) return;
        int level = Math.max(1, Math.min(5, villager.getVillagerData().level()));
        ...
        for (var offer : new ArrayList<>(offers)) {
            ...
            var book = books.stream().filter(b -> ItemStack.isSameItemSameComponents(b.stack(), offer.getResult())).findFirst();
    ...
    private static void merge(MerchantOffers offers, ItemStack result, boolean book, int basePrice, int level, int unlock) {
        ...
        for (int i = 0; i < offers.size(); i++) {
            ...
            if (offer.getBaseCostA().is(ModItems.NETHER_CORE.get())
                    ...
                    && ItemStack.isSameItemSameComponents(offer.getResult(), result)) {
                index = i;
                break;
            }
        }
        ...
        var updated = new MerchantOffer(new ItemCost(ModItems.NETHER_CORE.get(), price), book ? Optional.of(new ItemCost(Items.BOOK)) : Optional.empty(), result.copy(), uses, maxUses, xp, 0F, demand);
        if (old != null) {
