        ...
        for (int radius = 0; radius < 16; radius++) {
            for (int dx = -radius; dx <= radius; dx++) for (int dz = -radius; dz <= radius; dz++) {
                ...
                for (int y = floor + 1; y < city.maxY(); y++) {
                    ...
                    Villager dealer = EntityTypes.VILLAGER.create(level.getLevel(), EntitySpawnReason.STRUCTURE);
                    if (dealer == null) return;
                    dealer.snapTo(x + 0.5, y, z + 0.5);
                    if (!level.noCollision(dealer)) continue;
                    dealer.setVillagerData(dealer.getVillagerData().withProfession(
                            BuiltInRegistries.VILLAGER_PROFESSION.wrapAsHolder(ModVillagers.RELIC_DEALER.get())));
