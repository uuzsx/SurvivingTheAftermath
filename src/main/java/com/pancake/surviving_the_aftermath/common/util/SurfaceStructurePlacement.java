...
public final class SurfaceStructurePlacement {
    ...
    public static Optional<BlockPos> findOrigin(Structure.GenerationContext context, StructureTemplate template,
        ...
        for (var shift : offsets) {
            ...
            if (y == INVALID_GROUND || !context.validBiome().test(context.biomeSource().getNoiseBiome(net.minecraft.core.QuartPos.fromBlock(x), net.minecraft.core.QuartPos.fromBlock(y), net.minecraft.core.QuartPos.fromBlock(z), context.randomState().sampler()))) continue;
            var candidate = plan(template,rotation,x,z,groundOffset,context.heightAccessor().getMinBuildHeight(),context.heightAccessor().getMaxBuildHeight(),ground);
