...
public class RaidModuleProvider extends AftermathModuleProviders<BaseRaidModule> {
    ...
    private BaseRaidModule create(RaidDifficulty difficulty, int rewardTime, ItemWeightedModule rewards) {
        ...
        for (int index = 0; index < difficulty.waves(); index++) {
            ...
            enemies.add(group(net.minecraft.world.entity.EntityTypes.PIGLIN, counts[0], wave == 1, sword, MELEE_ARMOR[index], min, max));
            enemies.add(group(net.minecraft.world.entity.EntityTypes.PIGLIN, counts[1], wave == 1, Items.CROSSBOW, RANGED_ARMOR[index], min, max));
            if (counts[2] > 0) enemies.add(new EntityInfoModule(net.minecraft.world.entity.EntityTypes.HOGLIN, new IntegerAmountModule(counts[2])));
            if (counts[3] > 0) enemies.add(group(net.minecraft.world.entity.EntityTypes.PIGLIN_BRUTE, counts[3], false, axe, BRUTE_ARMOR[index], 4, 4));
