...
public class ClientForgeEvent {
    ...
    public static void netherRaidProgress(CustomizeGuiOverlayEvent.BossEventProgress event) {
        ...
        if (aftermath != null) {
            ...
            //渲染进度条框
            graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, resource, (graphics.guiWidth() - frameWidth) / 2, frameY,
                    0, frameOffset, frameWidth, frameHeight, 256, 256);
            //渲染进度条
            graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, resource, (graphics.guiWidth() - barWidth) / 2, frameY + barOffset,
                    0, 0, (int) (barWidth * event.getBossEvent().getProgress()), barHeight, 256, 256);
            if (labels != null) {
                graphics.centeredText(font, labels.difficulty(), graphics.guiWidth() / 2, titleY,
                        RaidBarText.difficultyColor(System.nanoTime() / 1_000_000L));
                graphics.centeredText(font, labels.wave(), graphics.guiWidth() / 2, waveY, 0xFFFFFFFF);
                ...
                graphics.centeredText(font, bossEvent.getName(), graphics.guiWidth() / 2, event.getY() + 2, 0xFFFFFFFF);
