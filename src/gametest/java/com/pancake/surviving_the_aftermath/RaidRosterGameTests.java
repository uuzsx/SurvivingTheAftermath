...
public final class RaidRosterGameTests {
    ...
    public static void sharedRosterQuotasAndReload(GameTestHelper h) {
        ...
        var legacyTag=(CompoundTag)EntityInfoModule.CODEC.encodeStart(NbtOps.INSTANCE,new EntityInfoModule(EntityType.PIGLIN,new com.pancake.surviving_the_aftermath.common.module.amount.IntegerAmountModule(1))).result().orElseThrow();
    ...
    public static void rosterCrossbowUsesNativeAi(GameTestHelper h) {
        ...
        archer.moveTo(Vec3.atBottomCenterOf(origin));
        var target=EntityType.IRON_GOLEM.create(level,EntitySpawnReason.EVENT);
        target.setNoAi(true);target.moveTo(Vec3.atBottomCenterOf(origin.east(6)));
