...
public final class NetherCoreGameTests {
    ...
    private static FakePlayer player(GameTestHelper h) {
        ...
        player.moveTo(Vec3.atCenterOf(h.absolutePos(new BlockPos(3, 6, 3))));
        return player;
    }
    ...
    private static ItemStack namedCore() {
        ...
        stack.setHoverName(Component.literal("precious_core"));
        return stack;
    }

    @GameTest(template = "stability_empty", batch = "corealwaysreturnsandcanbethrownagain", timeoutTicks = 200)
    ...
    @GameTest(template = "stability_empty", batch = "coreflightsurvivessavereload", timeoutTicks = 200)
    public static void coreFlightSurvivesSaveReload(GameTestHelper h) {
        ...
        original.saveWithoutId(saved); original.discard();
    ...
    @GameTest(template = "stability_empty", batch = "coreusetrackslivingdealerandticksnaturally", timeoutTicks = 200)
    public static void coreUseTracksLivingDealerAndTicksNaturally(GameTestHelper h) {
        cleanup(h);var player = player(h);
        var dealer = EntityType.VILLAGER.create(h.getLevel());
        dealer.moveTo(player.getX()+10, player.getY(), player.getZ());dealer.setNoAi(true);
    ...
    @GameTest(template = "stability_empty", batch = "corecreativeandmissingtargetdonotduplicate", timeoutTicks = 200)
    ...
    @GameTest(template = "stability_empty", batch = "corestopsbeforewallsandneverdespawns", timeoutTicks = 200)
    ...
    public static void coreBackgroundSearchAndCancellation(GameTestHelper h) throws Exception {
        ...
        var city=h.getLevel().registryAccess().registryOrThrow(net.minecraft.core.registries.Registries.STRUCTURE).getOrThrow(ModStructures.CITY);
