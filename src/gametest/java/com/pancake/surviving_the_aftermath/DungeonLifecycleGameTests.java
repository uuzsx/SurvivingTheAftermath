...
public class DungeonLifecycleGameTests {
    ...
    static AutoCloseable arena(GameTestHelper h) {
        ...
        var structure = level.registryAccess().registryOrThrow(Registries.STRUCTURE).get(ModStructures.NETHER_RAID);
    ...
    private static void portalLifecycle(GameTestHelper h, Direction.Axis axis, Item item, RaidDifficulty difficulty) throws Exception {
        ...
        try (var ignored = arena(h)) {
            ...
            PortalShape.findEmptyPortalShape(level, pos, axis).orElseThrow().createPortalBlocks();
    ...
    public static void goldenRecipeRequiresBothIngredients(GameTestHelper h) {
        ...
        check(h.getLevel().getRecipeManager().getRecipeFor(RecipeType.CRAFTING, grid.asCraftInput(), h.getLevel()).map(r -> !r.value().assemble(grid.asCraftInput(), h.getLevel().registryAccess()).is(ModItems.GOLDEN_FLINT_AND_STEEL.get())).orElse(true), "Gold alone crafted the igniter");
        grid.setItem(3, new ItemStack(Items.FLINT_AND_STEEL));
        check(h.getLevel().getRecipeManager().getRecipeFor(RecipeType.CRAFTING, grid.asCraftInput(), h.getLevel()).isEmpty(), "Old flint-and-steel ingredient still accepted");
        grid.setItem(3, new ItemStack(Items.FLINT));
        var recipe = h.getLevel().getRecipeManager().getRecipeFor(RecipeType.CRAFTING, grid.asCraftInput(), h.getLevel()).orElseThrow();
