...
public final class PortalEntranceGameTests {
    ...
    public static void entranceAuthenticTemplateAndWalk(GameTestHelper h) {
        var level=h.getLevel();var template=level.getStructureTemplateManager().getOrCreate(SurvivingTheAftermath.asResource("nether_invasion_portal"));
        int cases=0;
        for(Rotation rotation:Rotation.values()) {
            ...
            for(var type:List.of(EntityTypes.PIGLIN,EntityTypes.PIGLIN_BRUTE,EntityTypes.HOGLIN)) {
                var mob=mob(h,type);
    ...
    public static void entranceQueueCountsEquipmentAndReload(GameTestHelper h) {
        ...
        player.snapTo(Vec3.atCenterOf(base.south(4)));level.addNewPlayer(player);
    ...
    private static Mob restoredMob(GameTestHelper h,Mob mob) {
        ...
        mob.save(output);
        return (Mob)EntityType.loadEntityRecursive(net.minecraft.world.level.storage.TagValueInput.create(net.minecraft.util.ProblemReporter.DISCARDING,h.getLevel().registryAccess(),output.buildResult()),h.getLevel(),EntitySpawnReason.EVENT,e->e);
    }
    ...
    public static void entranceObstructionAndAiRecovery(GameTestHelper h) {
        ...
        var mob=mob(h,EntityTypes.PIGLIN);mob.snapTo(Vec3.atCenterOf(base));
        ...
        var blocked=mob(h,EntityTypes.HOGLIN);check(PortalEntrance.place(level,blocked,portal,Vec3.atCenterOf(base.east(4))).isEmpty(),"Blocked door teleported entrant outside");blocked.discard();
        var player=new FakePlayer(level,new GameProfile(UUID.randomUUID(),"blocked_entrance"));player.snapTo(Vec3.atCenterOf(base.east(4)));level.addNewPlayer(player);
