...
public final class RaidDifficultyGameTests {
    ...
    static void verify(Mob mob, RaidDifficulty mode, int wave) {
        ...
        for(var effect:mob.getActiveEffects()) {
            if(mob.getMainHandItem().is(Items.CROSSBOW)) check(effect.getEffect()!=MobEffects.STRENGTH,"Strength has no effect on crossbow shots");
    ...
    public static void difficultyLiveWavesAndPersistence(GameTestHelper h) {
        ...
        player.snapTo(pos.south(7).getCenter());level.addNewPlayer(player);
    ...
    public static void combatBuffsAreEffectiveAndInherited(GameTestHelper h) {
        ...
        for(var type:List.of(EntityType.PIGLIN,EntityType.PIGLIN_BRUTE,EntityType.HOGLIN,EntityType.MAGMA_CUBE,EntityType.BLAZE,EntityType.GHAST)) {
            for(int wave:List.of(6,7,9,10,11,12,13)) for(int seed=0;seed<24;seed++) {
                ...
                if(type==EntityType.MAGMA_CUBE && wave==13) {
                    var child=EntityType.MAGMA_CUBE.create(level,EntitySpawnReason.EVENT);
        ...
        var attacker=EntityType.PIGLIN_BRUTE.create(level,EntitySpawnReason.EVENT);
        var plain=EntityType.PIGLIN_BRUTE.create(level,EntitySpawnReason.EVENT);
        var target=EntityType.IRON_GOLEM.create(level,EntitySpawnReason.EVENT);
        attacker.snapTo(h.absolutePos(new BlockPos(4,3,4)).getCenter());plain.snapTo(attacker.position());target.snapTo(attacker.position().add(1,0,0));
