...
public final class TerrainPlacementGameTests {
    ...
    public static void terrainPlansCheckFootprint(GameTestHelper h) {
        for (String id : templates()) for (Rotation rotation : Rotation.values()) {
            var template = h.getLevel().getStructureTemplateManager().getOrCreate(SurvivingTheAftermath.asResource(id));
    ...
    private static void placeChunk(GameTestHelper h, String id, Rotation rotation, int index, boolean reload) {
        ...
        var template = level.getStructureTemplateManager().getOrCreate(SurvivingTheAftermath.asResource(id));
        ...
        var origin = SurfaceStructurePlacement.plan(template, rotation, center.getX(), center.getZ(), offset,
                level.getMinY(), (level.getMinY() + level.getHeight()), (x, z) -> center.getY()).orElseThrow();
        var piece = new AbstractStructure.Piece(ModStructurePieceTypes.NETHER_RAID.get(), level.getStructureTemplateManager(),
        ...
        if (reload) {
            ...
            check(tag.getBooleanOr("SurfaceGrounded", false), "Ground placement flag was not saved");
        ...
        var chunk = new BoundingBox(chunkPos.getMinBlockX(), level.getMinY(), chunkPos.getMinBlockZ(),
                chunkPos.getMaxBlockX(), (level.getMinY() + level.getHeight()) - 1, chunkPos.getMaxBlockZ());
    ...
    public static void terrainLegacyPiecesKeepOldPlacement(GameTestHelper h) {
        ...
        var template = level.getStructureTemplateManager().getOrCreate(SurvivingTheAftermath.asResource("camp"));
        var piece = new AbstractStructure.Piece(ModStructurePieceTypes.NETHER_RAID.get(), level.getStructureTemplateManager(),
    ...
    public static void terrainRealNoiseSiteSelection(GameTestHelper h) {
        ...
        long seed = 20260924L;
        var state = net.minecraft.world.level.levelgen.RandomState.create(lookup.lookupOrThrow(net.minecraft.core.registries.Registries.NOISE), seed, noise.value());
        var climate = state.createClimateSampler(net.minecraft.world.level.levelgen.densityfunction.SamplerContext.EMPTY_UNCACHED);
        for (String id : List.of("city", "nether_invasion_portal")) {
            ...
            for (int i = 0; i < 32; i++) {
                ...
                var context = new net.minecraft.world.level.levelgen.structure.Structure.GenerationContext(
                        lookup, generator, generator.getBiomeSource(), climate, state, level.getStructureTemplateManager(), seed, chunk, level, b -> true);
                ...
                var forbidden = new net.minecraft.world.level.levelgen.structure.Structure.GenerationContext(
                        lookup, generator, generator.getBiomeSource(), climate, state, level.getStructureTemplateManager(), seed, chunk, level, b -> false);
    ...
    public static void terrainFullCityFoundationAndTransition(GameTestHelper h) {
        ...
        var origin = SurfaceStructurePlacement.plan(template, Rotation.CLOCKWISE_90, center.getX(), center.getZ(),
                0, level.getMinY(), level.getMinY() + level.getHeight(), (x, z) -> floor).orElseThrow();
    ...
    public static void terrainCitySettingsAndGentleSlope(GameTestHelper h) {
        ...
        var template = level.getStructureTemplateManager().getOrCreate(SurvivingTheAftermath.asResource("city"));
        ...
        var climate = state.createClimateSampler(net.minecraft.world.level.levelgen.densityfunction.SamplerContext.EMPTY_UNCACHED);
            ...
            search: for (int radius = 0; radius <= 6; radius++) {
                for (int rx = -radius; rx <= radius; rx++) for (int rz = -radius; rz <= radius; rz++) {
                    ...
                    queried++;
                    var context = new net.minecraft.world.level.levelgen.structure.Structure.GenerationContext(lookup, generator, generator.getBiomeSource(), climate, state, manager, seed, candidate, level, b -> b.is(com.pancake.surviving_the_aftermath.common.init.ModTags.HAS_CITY));
