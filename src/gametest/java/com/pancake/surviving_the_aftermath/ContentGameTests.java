...
public final class ContentGameTests {
    ...
    private static ItemStack enchanted(GameTestHelper h, Item item, String name, int rank) {
        ...
        stack.enchant(ModEnchantments.get(name), rank);
        return stack;
    }
        ...
        for (String locale : List.of("zh_cn", "en_us")) {
            try (var stream = ContentGameTests.class.getResourceAsStream("/assets/surviving_the_aftermath/lang/" + locale + ".json"); var reader = new InputStreamReader(stream, StandardCharsets.UTF_8)) {
                ...
                for (String id : LegacyEnchantments.NAMES) {
                    var enchantment=ModEnchantments.get(id);
                    check(lang.has("enchantment.surviving_the_aftermath." + id), "Missing enchantment translation: " + id);
                    check(enchantment.getMaxLevel() > 0, "Unusable enchantment " + id);
                }
            }
        }
        check(!ModEnchantments.get("clean_water").isCompatibleWith(net.minecraft.world.item.enchantment.Enchantments.SHARPNESS), "Clean Water stacked with Sharpness");
    ...
    public static void restoredFoodConsumption(GameTestHelper h) {
        ...
        for(int i=0;i<items.size();i++) {
            ...
            var stack=new ItemStack(items.get(i),2); var food=stack.getFoodProperties(p);
            check(food.getNutrition()==nutrition[i] && Math.abs(food.getNutrition()*2*food.getSaturationModifier()-saturation[i])<.001, "Food values changed for " + items.get(i));
    ...
    private static void effect(Player p, MobEffect effect, int ticks, int amplifier) {
    ...
    public static void cowardicePricesAndLegacyTrades(GameTestHelper h) throws Exception {
        ...
        p.addEffect(new MobEffectInstance(ModMobEffects.COWARDICE.get(),100,0)); update.invoke(v,p);
        check(offers.get(0).getCostA().getCount()==21,"Cowardice did not restore the original price penalty");
        offers.get(0).resetSpecialPriceDiff();p.removeEffect(ModMobEffects.COWARDICE.get());update.invoke(v,p);
        check(offers.get(0).getCostA().getCount()==16,"Cowardice price leaked to an unaffected player");
        p.addEffect(new MobEffectInstance(ModMobEffects.COWARDICE.get(),100,1));update.invoke(v,p);
        check(offers.get(0).getCostA().getCount()==21,"Second level penalty differs from legacy rule");
        for(String profession:List.of("farmer","butcher")) {
            var seller=(Villager)mob(h,EntityType.VILLAGER);
            seller.setVillagerData(seller.getVillagerData().setProfession(BuiltInRegistries.VILLAGER_PROFESSION.get(new net.minecraft.resources.ResourceLocation(profession))).setLevel(4));
    ...
    public static void restoredDiscInJukebox(GameTestHelper h) {
        ...
        var box=(JukeboxBlockEntity)h.getBlockEntity(pos);box.setItem(0,new ItemStack(ModItems.MUSIC_DISK_ORCHELIAS_VOX.get()));
        check(box.isRecordPlaying(),"Restored music disc cannot play in a jukebox");
        check(((RecordItem)ModItems.MUSIC_DISK_ORCHELIAS_VOX.get()).getAnalogOutput()==15,"Music disc lost comparator signal");
        check(((RecordItem)ModItems.MUSIC_DISK_ORCHELIAS_VOX.get()).getLengthInTicks()==5380,"Music disc length mismatch");
        box.removeItem(0,1);check(!box.isRecordPlaying(),"Jukebox did not stop on disc removal");h.succeed();
    }
    ...
    public static void restoredCombatEnchantments(GameTestHelper h) {
        ...
        var attacker=mob(h,EntityType.ZOMBIE);p.invulnerableTime=0;p.hurt(level.damageSources().mobAttack(attacker),2);
        check(attacker.getDeltaMovement().horizontalDistanceSqr()>0,"Counter Attack failed to push the attacker");attacker.discard();
        var bow=enchanted(h,Items.BOW,"ranger",1);var use=new LivingEntityUseItemEvent.Tick(p,bow,bow.getUseDuration());MinecraftForge.EVENT_BUS.post(use);
        check(use.getDuration()==bow.getUseDuration()-1,"Ranger did not accelerate bow drawing");h.succeed();
    }

    @GameTest(template = "stability_empty")
    public static void restoredGrowthEnchantments(GameTestHelper h) {
        var ordinary = new ItemStack(Items.STICK);
        ordinary.getAttributeModifiers(EquipmentSlot.MAINHAND);
        check(!ordinary.hasTag(), "Attribute lookup mutated an unrelated item");
        ...
        float growth=sword.getOrCreateTag().getFloat(LegacyEnchantments.DEVOURED_DATA);
        check(growth>0 && growth<=5,"Devoured kill growth missing or above cap");
        var attack=sword.getAttributeModifiers(EquipmentSlot.MAINHAND).get(Attributes.ATTACK_DAMAGE).stream().filter(m->m.getId().equals(java.util.UUID.fromString("412c831f-22ea-43b8-b74b-d172019ad3d2"))).findFirst().orElseThrow();
        check(Math.abs(attack.getAmount()-growth)<.001,"Devoured data has no damage modifier");
        var helmet=enchanted(h,Items.LEATHER_HELMET,"life_tree",4);
        var health=helmet.getAttributeModifiers(EquipmentSlot.HEAD).get(Attributes.MAX_HEALTH).stream().findFirst().orElseThrow();
        check(Math.abs(health.getAmount()-.4)<.001,"Life Tree health modifier missing");
        h.succeed();
    }
}
