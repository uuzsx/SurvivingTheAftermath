...
public final class ContentGameTests {
    ...
    public static void restoredRegistryAndLanguage(GameTestHelper h) throws Exception {
        ...
        for (String locale : List.of("zh_cn", "en_us")) {
            try (var stream = ContentGameTests.class.getResourceAsStream("/assets/surviving_the_aftermath/lang/" + locale + ".json"); var reader = new InputStreamReader(stream, StandardCharsets.UTF_8)) {
                ...
                for (String id : ids) {
                    var item = BuiltInRegistries.ITEM.getValue(SurvivingTheAftermath.asResource(id));
                    ...
                    check(ContentGameTests.class.getResource("/assets/surviving_the_aftermath/items/"+id+".json") != null, "Missing modern item definition " + id);
                }
        ...
        try (var stream=ContentGameTests.class.getResourceAsStream("/assets/minecraft/items/enchanted_book.json");var reader=new InputStreamReader(stream,StandardCharsets.UTF_8)) {
            var cases=JsonParser.parseReader(reader).getAsJsonObject().getAsJsonObject("model").getAsJsonArray("cases");
            check(cases.size()==30,"Incomplete legacy book texture/rank coverage");
            for(var entry:cases) {
                var stored=net.minecraft.world.item.enchantment.ItemEnchantments.CODEC.parse(h.getLevel().registryAccess().createSerializationContext(com.mojang.serialization.JsonOps.INSTANCE),entry.getAsJsonObject().get("when")).getOrThrow();
                check(stored.keySet().size()==1,"Invalid enchantment book model selector");
            }
        }
        h.succeed();
    }
    ...
    public static void restoredFoodConsumption(GameTestHelper h) {
        ...
        for(int i=0;i<items.size();i++) {
            ...
            switch(i) {
                case 1 -> effect(p,MobEffects.STRENGTH,4800,0);
                case 2 -> effect(p,MobEffects.HEALTH_BOOST,4800,0);
                case 3 -> { effect(p,MobEffects.HEALTH_BOOST,3600,1);effect(p,MobEffects.NAUSEA,60,0); }
    ...
    public static void cowardicePricesAndLegacyTrades(GameTestHelper h) throws Exception {
        ...
        check(offers.size()==3 && offers.getFirst().getBaseCostA().is(ModItems.NETHER_CORE.get()) && offers.getFirst().getResult().is(Items.ENCHANTED_BOOK),"Relic dealer cannot exchange cores for enchanted books");
        ...
        for(String profession:List.of("farmer","butcher")) {
            ...
            seller.setVillagerData(seller.getVillagerData().withProfession(BuiltInRegistries.VILLAGER_PROFESSION.get(net.minecraft.resources.Identifier.withDefaultNamespace(profession)).orElseThrow()).withLevel(4));
    ...
    public static void restoredDiscInJukebox(GameTestHelper h) {
        ...
        var box=(JukeboxBlockEntity)h.getBlockEntity(pos, JukeboxBlockEntity.class);box.setTheItem(new ItemStack(ModItems.MUSIC_DISK_ORCHELIAS_VOX.get()));
    ...
    public static void restoredCombatEnchantments(GameTestHelper h) {
        ...
        for(String name:List.of("clean_water","bloodthirsty","frantic","moon","sun","execute")) {
            ...
            target.hurtServer(level,level.damageSources().playerAttack(p),damage);
            switch(name) {
                ...
                case "moon" -> check(Math.abs(before-target.getHealth()-(level.isBrightOutside()?7:4))<.01,"Moon legacy day condition missing");
                case "sun" -> check(Math.abs(before-target.getHealth()-(level.isDarkOutside()?7:4))<.01,"Sun legacy night condition missing");
        ...
        var attacker=mob(h,EntityType.ZOMBIE);p.invulnerableTime=0;p.hurtServer(level,level.damageSources().mobAttack(attacker),2);
    ...
    public static void restoredGrowthEnchantments(GameTestHelper h) {
        ...
        for(int i=0;i<32;i++){var target=mob(h,EntityType.COW);target.hurtServer(h.getLevel(),h.getLevel().damageSources().playerAttack(p),100);target.discard();}
        float growth=sword.getOrDefault(DataComponents.CUSTOM_DATA,CustomData.EMPTY).copyTag().getFloatOr(LegacyEnchantments.DEVOURED_DATA,0);
