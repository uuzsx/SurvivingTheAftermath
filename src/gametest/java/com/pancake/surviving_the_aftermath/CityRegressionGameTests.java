...
public final class CityRegressionGameTests {
    ...
    public static void terrainSamplerMatchesVanilla(GameTestHelper h) {
        ...
        for(long seed:new long[]{0,42,20260924}) {
            var random=RandomState.create(lookup.lookupOrThrow(Registries.NOISE),seed,noise.value());
            var context=new Structure.GenerationContext(lookup,generator,source,random.createClimateSampler(net.minecraft.world.level.levelgen.densityfunction.SamplerContext.EMPTY_UNCACHED),random,level.getStructureTemplateManager(),seed,new ChunkPos(0,0),level,b->true);
    ...
    public static void undergroundStructuresDoNotDisableCitySupport(GameTestHelper h) {
        ...
        try {
            ...
            var underground=new AbstractStructure.Piece(ModStructurePieceTypes.CONSTRUCTION_1.get(),level.getStructureTemplateManager(),
