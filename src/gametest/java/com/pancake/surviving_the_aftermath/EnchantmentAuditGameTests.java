...
public final class EnchantmentAuditGameTests {
    ...
    public static void enchantmentCombatLevelsAndConditions(GameTestHelper h) throws Exception {
        ...
        try {
            ...
            for(int rank=1;rank<=3;rank++) {
                ...
                p.damageCooldownTime=0;p.hurtServer(level,level.damageSources().mobAttack(attacker),1);
    ...
    public static void relicCatalogueProgression(GameTestHelper h) throws Exception {
        ...
        int[] sent={0,0};
        var customer=new FakePlayer(h.getLevel(),new GameProfile(UUID.randomUUID(),"relic_menu_test")) {
            @Override public void sendMerchantOffers(int container, MerchantOffers offers, int tier, int xp, boolean progress, boolean restock) {
                sent[0]=offers.size();sent[1]=offers.get(0).getCostA().getCount();
            }
        };
        customer.containerMenu=new net.minecraft.world.inventory.MerchantMenu(1,customer.getInventory(),v);
        customer.addEffect(new MobEffectInstance(ModMobEffects.COWARDICE,200,0));
        v.setTradingPlayer(customer);
        ...
        for(int tier=1;tier<=5;tier++) {
            ...
            if(tier==1) {
                ...
                check(v.getVillagerData().level()==2 && sent[0]==6,"Immediate upgrade did not refresh open menu");
                // Apprentice rank-I price is 2 cores; Cowardice adds its minimum 1-core penalty.
                check(sent[1]==3,"Client price packet omitted Cowardice after career discount");
            }
            ...
            if(tier<5 && v.getVillagerData().level()==tier)upgrade.invoke(v ,h.getLevel());
        }
        check(v.getOffers().stream().filter(o->!o.getResult().is(Items.ENCHANTED_BOOK)).count()==5,"Dealer should have five foods");
        v.setTradingPlayer(null);customer.removeEffect(ModMobEffects.COWARDICE);
        ...
        v.discard();System.out.println("RELIC LIVE MENU CHECK: immediate 26.3 upgrade sends six offers and final Cowardice-adjusted price");System.out.println("RELIC CATALOGUE CHECK: five tiers 3/6/9/12/15, discounts, inventory retention and timed restock");h.succeed();
    }
