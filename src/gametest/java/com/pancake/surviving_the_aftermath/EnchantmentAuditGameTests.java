...
public final class EnchantmentAuditGameTests {
    ...
    private static void hit(GameTestHelper h, LivingEntity target, Player p, float amount) {
        ...
        target.hurt(h.getLevel().damageSources().playerAttack(p), amount);
    }
    private static ItemStack book(GameTestHelper h, String name, int rank) {
        return net.minecraft.world.item.EnchantedBookItem.createForEnchantment(new net.minecraft.world.item.enchantment.EnchantmentInstance(ModEnchantments.get(name),rank));
    }
    ...
    public static void enchantmentBooksWorkInAnvil(GameTestHelper h) {
        ...
        sharp.enchant(net.minecraft.world.item.enchantment.Enchantments.SHARPNESS,1);
    ...
    public static void enchantmentExecuteRespectsTotems(GameTestHelper h) {
        ...
        var target=mob(h,EntityType.COW);
        ...
        for(int rank=1;rank<=3;rank++) {
            ...
            target=mob(h,EntityType.COW);target.setHealth(threshold+.05F);hit(h,target,p,.025F);
        ...
        target=mob(h,EntityType.COW);target.setHealth(.45F);hit(h,target,p,.1F);
    ...
    public static void enchantmentEquipmentAndGrowthLifecycle(GameTestHelper h) throws Exception {
        ...
        for(int i=0;i<160;i++){var target=mob(h,EntityType.COW);hit(h,target,p,100);target.discard();}
        syncEquipment(p);
        ...
        var restored=ItemStack.of(sword.save(new net.minecraft.nbt.CompoundTag()));
    ...
    public static void enchantmentCombatLevelsAndConditions(GameTestHelper h) throws Exception {
        var p=player(h);var level=h.getLevel();long previous=level.getDayTime();
        try {
            for(int rank=1;rank<=4;rank++) {
                ...
                for(boolean nether:List.of(false,true)) {
                    var target=mob(h,nether?EntityType.HOGLIN:EntityType.COW);float health=target.getHealth();hit(h,target,p,1);
                ...
                for(String name:List.of("moon","sun"))for(long time:List.of(1000L,18000L)) {
                    ...
                    var target=mob(h,EntityType.COW);hit(h,target,p,1);
                    boolean active=name.equals("moon")?level.isDay():level.isNight();
            ...
            for(int rank=1;rank<=3;rank++) {
                ...
                var target=mob(h,EntityType.COW);hit(h,target,p,4);
                check(Math.abs(p.getHealth()-(10+.2*rank))<.001,"Bloodthirsty rank mismatch");target.discard();
                target=mob(h,EntityType.COW);target.addEffect(new MobEffectInstance(MobEffects.ABSORPTION,200,1));target.setAbsorptionAmount(5);p.setHealth(10);hit(h,target,p,4);
            ...
            for(float health:List.of(20F,10F,2F)) {
                p.setHealth(health);var target=mob(h,EntityType.COW);hit(h,target,p,1);
            ...
            for(int rank=1;rank<=3;rank++) {
                ...
                var attacker=mob(h,EntityType.COW);attacker.setDeltaMovement(Vec3.ZERO);
                p.invulnerableTime=0;p.hurt(level.damageSources().mobAttack(attacker),1);
                check(Math.abs(attacker.getDeltaMovement().horizontalDistance()-rank*.5)<.001,"Counter Attack rank mismatch");attacker.discard();
            }
        } finally { level.setDayTime(previous);level.updateSkyBrightness(); }
    ...
    public static void relicLegacySaveMigration(GameTestHelper h) throws Exception {
        ...
        var expensiveFood=new MerchantOffer(new ItemStack(ModItems.NETHER_CORE.get(),10),ItemStack.EMPTY,new ItemStack(ModItems.HAMBURGER.get()),16,5,0F);
