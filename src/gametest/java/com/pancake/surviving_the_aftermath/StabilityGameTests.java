...
public class StabilityGameTests {
    ...
    public static void authoredLootAndDecorativeBarrels(GameTestHelper h) {
        ...
        check(level.getBlockEntity(origin.east(2)).saveWithoutMetadata().getString("LootTable").equals(AbstractStructure.STRUCTURE_SUPPLIES.toString()), "Treasure chest lost structure table");
        ...
        var generated = (RandomizableContainerBlockEntity) level.getBlockEntity(origin.east(2));
        check(!generated.isEmpty(), "Loaded structure loot table produced no items");
        level.setBlockAndUpdate(origin.east(6), Blocks.CHEST.defaultBlockState());
        var bookChest = (RandomizableContainerBlockEntity) level.getBlockEntity(origin.east(6));
        bookChest.setLootTable(SurvivingTheAftermath.asResource("chests/structure_book"), 123L);
        int enchantedBooks = 0;
        for (int slot = 0; slot < bookChest.getContainerSize(); slot++) {
            var stack = bookChest.getItem(slot);
            if (!stack.is(Items.ENCHANTED_BOOK)) continue;
            enchantedBooks++;
            check(net.minecraft.world.item.EnchantedBookItem.getEnchantments(stack).size() == 1, "Book subtable produced an empty or multi-enchantment book");
        }
        check(enchantedBooks == 1, "Book subtable did not yield exactly one enchanted book");
        h.succeed();
    }
    ...
    public static void otherStructureBarrelsReceiveLoot(GameTestHelper h) {
        ...
        for (int index = 1; index <= 2; index++) {
            ...
            ResourceLocation id = SurvivingTheAftermath.asResource("regression_construction_" + index);
            var template = level.getStructureManager().getOrCreate(id);
            template.fillFromWorld(level, origin, new Vec3i(3, 1, 1), false, Blocks.STRUCTURE_VOID);
            ...
            var piece = new AbstractStructure.Piece(type, level.getStructureManager(), id, origin, Rotation.NONE);
            var structure = level.registryAccess().registryOrThrow(Registries.STRUCTURE).get(key);
            structure.afterPlace(level, level.structureManager(), level.getChunkSource().getGenerator(), level.random,
                    BoundingBox.fromCorners(origin, origin.offset(3, 1, 1)), new ChunkPos(origin), new PiecesContainer(List.of(piece)));
            for (var pos : List.of(origin, origin.east(2))) {
                var barrel = (RandomizableContainerBlockEntity) level.getBlockEntity(pos);
                check(barrel != null && barrel.saveWithoutMetadata().getString("LootTable").equals(AbstractStructure.STRUCTURE_SUPPLIES.toString()),
                        "Construction " + index + " barrel missed loot at " + pos);
            }
            check(!level.getBlockEntity(origin.east(4)).saveWithoutMetadata().contains("LootTable"),
    ...
    public static void structureLootTablesActuallyGenerate(GameTestHelper h) {
        ...
        LootTable main = level.getServer().getLootData().getLootTable(AbstractStructure.STRUCTURE_SUPPLIES);
        ResourceLocation booksKey = SurvivingTheAftermath.asResource("chests/structure_book");
        LootTable books = level.getServer().getLootData().getLootTable(booksKey);
        ...
        for (int seed = 1; seed <= 4000; seed++) {
            ...
            for (var stack : drops) {
                ...
                if (stack.is(Items.ENCHANTED_BOOK)) {
                    sampledBooks++;
                    check(net.minecraft.world.item.EnchantedBookItem.getEnchantments(stack).size() == 1, "Structure produced an invalid enchanted book");
                }
            }
        }
        ...
        for (int seed = 1; seed <= 500; seed++) {
            ...
            check(drops.size() == 1 && drops.get(0).is(Items.ENCHANTED_BOOK), "Book subtable returned a non-book");
            var enchanted = net.minecraft.world.item.EnchantedBookItem.getEnchantments(drops.get(0));
            check(enchanted.size() == 1, "Book subtable returned no or multiple enchantments");
            var entry = enchanted.getCompound(0);
            String name = new ResourceLocation(entry.getString("id")).getPath();
            int rank = entry.getShort("lvl");
