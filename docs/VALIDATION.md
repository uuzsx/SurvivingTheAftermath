...
基础环境与 KubeJS 环境均通过 33 项 GameTest（32 项模组测试及 1 项原版测试，基础环境跳过 1 项脚本测试主体）。另通过实际客户端 Ogg 解码器及 6 项音频进度检查。新增内容、测试范围及限制见 [26.1.2 rc.4 说明](26.1.2-rc.4.md)。上表保留此前 rc.2/rc.3 的验证记录。

## rc.5 内容恢复验证

七个工程均重新执行数据生成、专用游戏测试服务端与构建。新增六项内容测试覆盖注册/翻译、食用效果、村民价格/交易、唱片机、战斗附魔和成长属性；完整范围见 [内容核对](CONTENT-AUDIT-rc.5.md)。

| Minecraft | GameTest 日志总数 |
| --- | --- |
| 1.20.1 | 31 |
| 1.21.1 | 32 |
| 1.21.2 | 32 |
...
基础环境每个工程有一项 KubeJS 条件测试在未安装时跳过主体；26.x 含一项原版测试。26.1.2 安装 KubeJS 后的 39 项也全部通过，并通过 Minecraft 客户端音频解码/定位检查。此版本未做人工客户端画面或耳听验收。产物哈希见 [rc.5 清单](rc.5-manifest.json)。

## rc.6 全版本修复验证

七个目标均通过数据生成、完整 GameTest、客户端实际音频解码与六个进度场景检查，以及 JAR 构建。

| Minecraft | 加载器 | GameTest 日志总数 | 音频检查 |
| --- | --- | --- | --- |
| 1.20.1 | Forge | 37 | 6/6 |
| 1.21.1 | NeoForge | 38 | 6/6 |
| 1.21.2 | NeoForge | 38 | 6/6 |
| 26.1.1 | NeoForge | 39 | 6/6 |
| 26.1.2 | NeoForge | 39 | 6/6 |
| 26.2 | NeoForge | 39 | 6/6 |
| 26.3 | NeoForge | 39 | 6/6 |

基础环境每个工程有一项 KubeJS 条件测试在未安装时跳过主体；26.x 含一项原版测试。1.20.1、1.21.1、26.1.2 另行安装 KubeJS，分别通过 37、38、39 项，执行了脚本测试主体。

1.20.1 通过真实岩浆怪分裂路径验证 Forge Mixin；交付 JAR 中也核对了该 Mixin 的混淆映射。所有 JAR 核对修复类、单声道音源、48 格衰减配置、物品注册资源、翻译及作者原始打火石贴图，排除测试类与测试数据。详细行为见 [全版本修复说明](ALL-VERSIONS-rc.6.md)，哈希见 [rc.6 清单](rc.6-manifest.json)。

未开展图形客户端实玩、真人联机及实际耳听验收；不能据自动测试承诺所有整合包都兼容。
